#!/bin/bash -l
# scripts/submit_train_swin_label_alpha.sh
#
...
# not touched. Also assumes the 7/31 validate-loop bugfix (mean_volume_scale
# -> matching SPECTDataset's (inp, lbl, scale) return) is already committed
# in train/train_swin_unetr.py -- without that fix this job will crash at
# the first validation epoch regardless of this flag.
#
...
#
# Submit with:  qsub scripts/submit_train_swin_label_alpha.sh
# Monitor with: qstat
#
