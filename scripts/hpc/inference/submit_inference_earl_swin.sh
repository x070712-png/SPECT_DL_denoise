#!/bin/bash -l
# scripts/hpc/inference/submit_inference_earl_swin.sh
#
# Runs the Swin UNETR XCAT-finetune LABEL-ALPHA checkpoint over the
# 10-seed EARL dataset. See submit_inference_earl_unet.sh for rationale.
#
# Submit with: qsub scripts/hpc/inference/submit_inference_earl_swin.sh
...
python3 -u scripts/run_inference_nema_earl.py \
    ...
    --model swin \
    --out_dir logs/denoised/swin_xcat_labelalpha_earl \
