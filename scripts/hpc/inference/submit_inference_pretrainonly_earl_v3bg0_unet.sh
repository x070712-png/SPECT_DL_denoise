#!/bin/bash -l
# scripts/hpc/inference/submit_inference_pretrainonly_earl_v3bg0_unet.sh
#
# Runs the ellipsoid-pretrained (NOT XCAT-finetuned) 3D U-Net label-alpha
# checkpoint over the 10-seed EARL v3-bg0 dataset, so its recovery
# coefficients can be compared against the XCAT-finetuned checkpoint's
# (submit_inference_earl_v3_bg0_unet.sh) -- isolates the effect of the
# XCAT fine-tuning stage on EARL generalisation.
#
# Submit with: qsub scripts/hpc/inference/submit_inference_pretrainonly_earl_v3bg0_unet.sh
...
#$ -l mem=16G
#$ -l tmpfs=10G
#$ -N spect_unet_infer_pretrainonly_earl_v3bg0
#$ -pe smp 1
...
python3 -u scripts/run_inference_nema_earl.py \
    ...
    --model unet \
    --out_dir logs/denoised/3d_unet_pretrainonly_earl_v3_bg0 \
