#!/bin/bash -l
# scripts/hpc/inference/submit_inference_dump_swin.sh
#
# Same as submit_inference_dump.sh but for the Swin UNETR checkpoint
# instead of 3D U-Net -- checkpoints/swin_unetr/best_model.pth, straight
# rerun of the same inference-dump step with --model swin.
#
# Uses --split test (NOT val) -- val is used for early-stopping/LR
# scheduling during training, so it is not a clean held-out set; test is
# never touched during training and is used for all reported quantitative
# results.
#
# Submit with:  qsub scripts/hpc/inference/submit_inference_dump_swin.sh
