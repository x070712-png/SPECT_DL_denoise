#!/bin/bash -l
# scripts/hpc/generate_data/submit_generate_eval_dataset_earl_v3_bg_ratio10.sh
#
...
# a 10:1 sphere:background ratio, an intermediate test between v2's
# background=84.4954 and bg0's background=0.0). Clean-reconstruction
# stats: mean=0.0772, max/mean ratio=146.24 -- still well off the
# ellipsoid target (mean=0.461, ratio=25.2), but much closer than bg0
# (mean=0.0096, ratio=1152.38).
#
# Same SIRF env + seed convention as submit_generate_eval_dataset_earl_v2.sh /
# submit_generate_eval_dataset_earl_v3_bg0.sh.
#
# Submit with: qsub scripts/hpc/generate_data/submit_generate_eval_dataset_earl_v3_bg_ratio10.sh
