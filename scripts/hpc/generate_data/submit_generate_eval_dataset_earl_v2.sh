#!/bin/bash -l
# scripts/hpc/generate_data/submit_generate_eval_dataset_earl_v2.sh
#
# Generates the 10-seed EARL v2 dataset from the jointly-calibrated
# phantom (sphere_conc=126.457, background_conc=84.4954 -- see
# generate_earl_phantom.py / submit_generate_earl_phantom_v2.sh) that
# fixes the background=0 domain-gap problem found in the original EARL
# phantom (data/earl_dataset).
#
# Same SIRF env + seed convention as submit_generate_eval_dataset.sh.
#
# Submit with: qsub scripts/hpc/generate_data/submit_generate_eval_dataset_earl_v2.sh
