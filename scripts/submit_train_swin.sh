...
#
# Submit with:  qsub scripts/submit_train_swin.sh
# Monitor with: qstat
#
# NOTE on wallclock time: no per-epoch timing for Swin UNETR yet (unlike
# U-Net, where Wei Miao's ~1min15s/epoch on an RTX 5090 gave a starting
# estimate). Swin UNETR is a heavier, transformer-based model with a smaller
# batch_size (2 vs 4), so per-epoch time is unknown until the first few
# epochs actually run on Myriad's hardware -- 48h kept as a conservative
# first guess, same as the U-Net job; check the log after a few epochs and
# adjust for future runs / fine-tuning jobs.
#
