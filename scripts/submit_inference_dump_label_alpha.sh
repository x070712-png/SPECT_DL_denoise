#!/bin/bash -l
# scripts/submit_inference_dump_label_alpha.sh
#
...
# trained 8/1 with --scale_label_by_alpha -- see submit_train_label_alpha.sh)
# instead of the baseline checkpoint. Straight rerun of the same
# inference-dump step, pointed at the new checkpoint and a separate
# output dir so it doesn't overwrite logs/denoised/3d_unet/ from the
# baseline run.
#
...
#
# Submit with:  qsub scripts/submit_inference_dump_label_alpha.sh
