...
CHECKPOINTS = {
    "unet_old":          {"label": "U-Net (old method)",        "denoised_dir": "logs/denoised/3d_unet"},
    "unet_label_alpha":  {"label": "U-Net (label x alpha)",      "denoised_dir": "logs/denoised/3d_unet_label_alpha"},
    "swin_old":          {"label": "Swin UNETR (old method)",    "denoised_dir": "logs/denoised/swin_unetr"},
    "swin_label_alpha":  {"label": "Swin UNETR (label x alpha)", "denoised_dir": "logs/denoised/swin_unetr_label_alpha"},
}
...
def main():
    ...
    for key in keys:
        ...
        for alpha in ALPHAS_ORDERED:
            ...
            all_diff_vals.append(np.abs(den_s - lbl_s).max())
            all_diff_vals.append(np.abs(inp_s - lbl_s).max())
    ...
    global_diff_absmax = max(all_diff_vals)
    print(f"Global intensity vmax = {global_vmax:.3f}, global diff scale = +/-{global_diff_absmax:.3f}")
    ...
    for key in keys:
        ...
        # one shared colorbar per column type (intensity, diff), figure-wide
        ...
        fig.suptitle(f"{CHECKPOINTS[key]['label']} -- {args.split} split, "
                      f"shared colour scale across all rows/checkpoints", fontsize=13)
