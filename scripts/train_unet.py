...
Adapted from Wei Miao's experiments/Unet/pre_training/train.py. Hyperparameters,
loss (0.5*MSE + 0.5*SSIM, window=5), optimizer and scheduler all match his
actual code (see config.py). The one thing swapped out is data loading: his
make_data_list_pre_training() + MONAI CacheDataset pipeline is replaced with
my own SPECTDataset(data_dir, split), since my data is organised into
per-alpha folders rather than his flat train/val/test layout. Because
SPECTDataset returns raw, un-normalised counts, this script also does the
per-volume mean normalisation his SaveMeand/DivideByScaled transforms would
otherwise handle.
"""
...
def parse_args():
    ...
    p.add_argument("--checkpoint_dir", type=str, default="checkpoints/3d_unet")
    p.add_argument("--log_dir", type=str, default="logs/3d_unet")
...
def main():
    ...
    lr_scheduler = ReduceLROnPlateau(optimizer, mode="min", factor=0.5, patience=3, min_lr=1e-6)

    mse_loss = torch.nn.MSELoss()
    # training loss: window=5 (see note #3 above)
    ...
    def per_volume_scale(x, eps=1e-8):
        """Mean of the noisy input per volume — used to bring pred/label
        back into count-domain for MSE/PSNR, and to normalise into a
        comparable range for the SSIM loss. Your SPECTDataset returns raw
        counts with no normalisation, so this step is required."""
        b = x.shape[0]
        return x.view(b, -1).mean(dim=1).clamp(min=eps).view(b, 1, 1, 1, 1)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for inp, lbl in tqdm(train_loader, desc=f"[Epoch {epoch:03d}] train"):
            ...
            # --- per-volume scale (required — see note #2 at top) ---
            scale = per_volume_scale(inp)
